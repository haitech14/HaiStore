...
insert into public.forum_events (title, starts_at, location, sort_order)
select * from (values
  ('Webinar: IA en flotas de impresión', now() + interval '5 days', 'Online · Zoom', 1),
  ('Taller Ricoh IM C Series', now() + interval '12 days', 'Lima · HaiStore', 2),
  ('Meetup distribuidores HaiTech', now() + interval '20 days', 'Online', 3)
) as v(title, starts_at, location, sort_order)
where not exists (select 1 from public.forum_events limit 1);

-- Hilos demo (solo si hay al menos un perfil)
do $$
declare
  v_author uuid;
  v_cat_ia uuid;
...
begin
  ...
  select id into v_cat_ia from public.forum_categories where slug = 'inteligencia-artificial';
  ...
  select id into v_cat_sec from public.forum_categories where slug = 'ciberseguridad';
  ...
  insert into public.forum_threads (
    category_id, author_id, title, slug, body, tags, view_count, reply_count, is_pinned, last_reply_at, last_reply_by
  )
  values
    (
      v_cat_ia,
      v_author,
      '¿GPT-4o cambiará la forma de atender soporte técnico?',
      'gpt-4o-soporte-tecnico',
      'Quiero saber cómo otros integradores usan IA para diagnóstico remoto de equipos Ricoh y reducción de visitas.',
      array['Inteligencia Artificial', 'Debate'],
      842,
      47,
      true,
    ...
    (
      v_cat_prog,
      v_author,
      'Integración de APIs Ricoh con ERP propio',
      'integracion-apis-ricoh-erp',
      'Comparto lecciones aprendidas conectando contadores y pedidos de tóner desde nuestro ERP.',
      array['Programación', 'Integración'],
      523,
      31,
      false,
    ...
    (
      v_cat_hw,
      v_author,
      'Comparativa IM C3010 vs IM C4010 para pymes',
      'comparativa-im-c3010-c4010',
      '¿Cuál recomiendan para oficinas de 15-25 usuarios con volumen mixto color/BN?',
      array['Hardware', 'Multifuncionales'],
      1204,
      58,
      false,
    ...
    (
      v_cat_sec,
      v_author,
      'Impresión segura: políticas que sí funcionan',
      'impresion-segura-politicas',
      'Mejoras concretas para evitar fugas de documentos en entornos corporativos.',
      array['Ciberseguridad', 'Guía'],
      391,
      22,
      false,
      now() - interval '1 day',
  ...
  update public.profiles
  set forum_title = coalesce(forum_title, 'Miembro activo'),
      forum_points = greatest(forum_points, 230),
      forum_level = greatest(forum_level, 23)
  where id = v_author;
end $$;
