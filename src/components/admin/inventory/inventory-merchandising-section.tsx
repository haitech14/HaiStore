...
import { ChevronDown, RefreshCw, ShoppingBag, TrendingUp } from 'lucide-react';
...
import { suggestCrossSellProductIds } from '@/lib/product-merchandising';
...
interface InventoryMerchandisingSectionProps {
  ...
  onChange: (patch: Pick<InventoryProduct, 'cross_sell_product_ids' | 'upsell_product_ids'>) => void;
}
...
export function InventoryMerchandisingSection({
  ...
  const handleSyncCrossSell = () => {
    ...
    onChange({
      cross_sell_product_ids: suggestedCrossSellIds,
    ...
    >
      <div className="space-y-5">
        <ProductMultiSelectField
          label="Ventas cruzadas (Cross Selling)"
          description="Tóner en el selector del hero; repuestos, tambor, accesorios y otros consumibles en el carrusel «Configura tu equipo»."
          selectedIds={form.cross_sell_product_ids ?? []}
          products={products}
          {...(form.id ? { excludeProductId: form.id } : {})}
          onChange={(cross_sell_product_ids) =>
            onChange({ cross_sell_product_ids, upsell_product_ids: form.upsell_product_ids ?? [] })
          }
        />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {suggestedCrossSellIds.length > 0 ? (
            <p className="text-xs text-muted-foreground">
              ...
              {suggestedCrossSellIds.length === 1 ? '' : 's'} (tóner, repuestos y accesorios detectados).
            </p>
          ) : (
            <p className="text-xs text-muted-foreground">
        ...
        <ProductMultiSelectField
          label="Upselling — Configura tu equipo"
          description="Tarjetas en el carrusel «Configura tu equipo»; al seleccionarlas se suman al total del equipo."
          selectedIds={form.upsell_product_ids ?? []}
          products={products}
          {...(form.id ? { excludeProductId: form.id } : {})}
          onChange={(upsell_product_ids) =>
            onChange({ upsell_product_ids, cross_sell_product_ids: form.cross_sell_product_ids ?? [] })
          }
        />

        <p className="flex items-start gap-2 rounded-md border border-border/60 bg-muted/15 px-3 py-2 text-xs text-muted-foreground">
          <TrendingUp className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
          Los productos de tóner aparecen en el selector del hero; repuestos y accesorios de venta cruzada
          y upselling se suman al total del equipo al pulsar «Agregar al total» en el carrusel.
        </p>
      </div>
    </InventoryFormSection>
  );
